...
def forward(
    ...
    if output_nsp_scores and output_lm_scores:
        lm_nsp_loss, lm_loss, nsp_loss, nsp_scores, lm_scores = dialog_encoder(
            tokens,
        ...
        lm_nsp_loss, lm_loss, nsp_loss, nsp_scores = dialog_encoder(
            tokens,
            sep_indices=sep_indices,
            sep_len=sep_len,
            token_type_ids=segments,
        ...
        lm_nsp_loss, lm_loss, nsp_loss, lm_scores = dialog_encoder(
            tokens,
            sep_indices=sep_indices,
            sep_len=sep_len,
            token_type_ids=segments,
        ...
        lm_nsp_loss, lm_loss, nsp_loss = dialog_encoder(
            tokens,
            sep_indices=sep_indices,
            sep_len=sep_len,
            token_type_ids=segments,
    ...
    if not evaluation:
        ...
        loss = (params["lm_loss_coeff"] * lm_loss) + (
...
if __name__ == "__main__":
    ...
    for epoch_id, idx, batch in batch_iter(dataloader, params):
        ...
        if iter_id % 10 == 0:
            ...
            print_format = "[%s][Ep: %.2f][Iter: %d][Time: %5.2fs][NSP + LM Loss: %.3g][LM Loss: %.3g][NSP Loss: %.3g]"
            print_info = [
                timestamp,
                cur_epoch,
                iter_id,
        ...
        # fire evaluation
        # print(
        #     "num iteration for eval",
        #     num_iter_epoch * (8 // params["sequences_per_image"]),
        # )
        if (
