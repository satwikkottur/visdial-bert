...
def forward(
    ...
    if "image_label" in batch:
        ...
        orig_features = (
            orig_features.unsqueeze(1)
            .unsqueeze(1)
            .expand(
                orig_features.shape[0],
                num_rounds,
                num_samples,
                orig_features.shape[1],
                orig_features.shape[2],
            )
            .contiguous()
        ).contiguous()
        orig_spatials = (
            orig_spatials.unsqueeze(1)
            .unsqueeze(1)
            .expand(
                orig_spatials.shape[0],
                num_rounds,
                num_samples,
                orig_spatials.shape[1],
                orig_spatials.shape[2],
            )
            .contiguous()
        ).contiguous()
        orig_image_label = batch["image_label"]
        orig_image_label = (
            orig_image_label.unsqueeze(1)
            .unsqueeze(1)
            .expand(
                orig_image_label.shape[0],
                num_rounds,
                num_samples,
                orig_image_label.shape[1],
            )
            .contiguous()
        ).contiguous()
        orig_image_mask = (
            orig_image_mask.unsqueeze(1)
            .unsqueeze(1)
            .expand(
                orig_image_mask.shape[0],
                num_rounds,
                num_samples,
                orig_image_mask.shape[1],
            )
            .contiguous()
        ).contiguous()
        orig_image_target = batch["image_target"]
        orig_image_target = (
            orig_image_target.unsqueeze(1)
            .unsqueeze(1)
            .expand(
                orig_image_target.shape[0],
                num_rounds,
                num_samples,
                orig_image_target.shape[1],
                orig_image_target.shape[2],
            )
            .contiguous()
        ).contiguous()
    ...
    if not evaluation:
        ...
        image_target = orig_image_target.view(
            -1, orig_image_target.shape[-2], orig_image_target.shape[-1]
        )
    ...
    if output_nsp_scores and output_lm_scores:
        lm_loss, img_loss, nsp_loss, nsp_scores, lm_scores = dialog_encoder(
            tokens,
            features,
            spatials,
        ...
        lm_loss, img_loss, nsp_loss, nsp_scores = dialog_encoder(
            tokens,
            features,
            spatials,
            sep_indices=sep_indices,
        ...
        lm_loss, img_loss, nsp_loss, lm_scores = dialog_encoder(
            tokens,
            features,
            spatials,
            sep_indices=sep_indices,
        ...
        lm_loss, img_loss, nsp_loss = dialog_encoder(
            tokens,
            features,
            spatials,
            sep_indices=sep_indices,
...
def visdial_evaluate(dataloader, params, eval_batch_size, dialog_encoder):
    ...
    batch_idx = 0
    with torch.no_grad():
        # we can fit approximately 500 sequences of length 256 in 8 gpus 
        # with 12 GB of memory during inference.
...
if __name__ == "__main__":
    ...
    dataloader = DataLoader(
        dataset,
        batch_size=params["batch_size"] // params["sequences_per_image"]
    ...
    scheduler = WarmupLinearScheduleNonZero(
        optimizer, warmup_steps=10000, t_total=200000
    )
    start_iter_id = 0
    ...
    if params["start_path"]:
        ...
        if not params["continue"]:
            ...
            scheduler = WarmupLinearScheduleNonZero(
                ...
                warmup_steps=10000,
                t_total=200000,
                last_epoch=pretrained_dict["iterId"],
            )
    ...
    for epoch_id, idx, batch in batch_iter(dataloader, params):
        ...
        loss, lm_loss, nsp_loss, img_loss = forward(
            dialog_encoder, batch, params, sample_size=sample_size
        )
        ...
        if iter_id % num_iter_epoch == 0 and iter_id > 0:
            torch.save(
                ...
                os.path.join(
                    params["save_path"], "visdial_dialog_encoder_%d.ckpt" % iter_id
                ),
            )
        ...
        if (
            ...
            all_metrics = visdial_evaluate(
                dataloader, params, eval_batch_size, dialog_encoder
            )
            for metric_name, metric_value in all_metrics.items():
                ...
                if "round" in metric_name:
                    viz.linePlot(
                        iter_id,
                        metric_value,
                        "Retrieval Round Val Metrics Round -"
                        + metric_name.split("_")[-1],
                        metric_name,
                    )
                else:
                    viz.linePlot(
                        iter_id, metric_value, "Retrieval Val Metrics", metric_name
                    )

            dataset.split = "train"
            # Decay the language model weight, every epoch by 0.5
            params["lm_loss_coeff"] /= 0.5
